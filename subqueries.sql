...
SELECT MAX(no_of_orders), region_name
FROM(
  SELECT r.name region_name, SUM(o.total_amt_usd) total_amt ,COUNT(*) no_of_orders
  FROM region r
  JOIN sales_reps s
  ON r.id=s.region_id
  JOIN accounts a
...
name which has bought the most standard_qty
...
GROUP BY 1
HAVING o.total > SELECT
  SELECT a.name , SUM(o.total_amt_usd) total_amt, SUM(o.standard_amt_usd) sum_standard
  FROM accounts a
  JOIN orders o
  ON a.id = o.account_id
  GROUP BY 1
